...
class StorageLayoutSpec extends AnyFunSuite {
    ...
    test("invalid months and keys are rejected at construction") {
        for (text <- Vector("2026-5", "2026-13", "26-05", ""))
            intercept[IllegalArgumentException](Month(text))
        for (text <- Vector("", "/nyc_raw/a", "s3a://nyc-taxi/a"))
            intercept[IllegalArgumentException](StorageKey(text))
    }
}
