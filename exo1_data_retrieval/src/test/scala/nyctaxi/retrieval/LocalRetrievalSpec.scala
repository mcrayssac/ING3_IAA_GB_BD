...
import nyctaxi.shared.{
    Fixtures, ParquetVerifier, ProvenanceFiles, SparkParquetVerifier,
    Verification
}
...
class LocalRetrievalSpec extends AnyFunSuite with BeforeAndAfterAll {
    ...
    override def beforeAll(): Unit = {
        ...
        Fixtures.directory { directory =>
            ...
            spark.range(10).selectExpr(
                "id",
                "concat('value-', id) as label",
                "cast(null as double) as missing"
            )
                .coalesce(1).write.option("compression", "uncompressed").parquet(destination.toString)
            bytes = Files.readAllBytes(
                Fixtures.children(destination).find(_.getFileName.toString.endsWith(".parquet")).get
            )
        }
    }
        ...
        Fixtures.directory { directory =>
            Fixtures.server((exchange, _) => Fixtures.respond(exchange, 200, bytes)) { (base, requests) =>
                Using.resource(new HttpDownload(policy)) { downloader =>
                    ...
                    val runner = new RetrievalRunner(
                        downloader,
                        verifier,
                        store,
                        month => base.resolve(month.value),
                        _ => ()
                    )
                    ...
                    val saved =
                        ProvenanceFiles.existing(file, Month("2026-05"), base.resolve("2026-05").toString).get
                    assert(saved.verification.rowCount == 10L &&
                        saved.verification.parquetSchema.contains("label"))
                    assert(saved.verification.sparkSchema.contains("missing") &&
                        Files.readAllBytes(file).sameElements(bytes))
    ...
    test("missing metadata, changed bytes, and interrupted promotion fail without HTTP or overwrites") {
        for (damage <- Vector("metadata", "invalid-metadata", "bytes", "marker", "file")) Fixtures.directory {
            directory =>
                Fixtures.server((exchange, _) => Fixtures.respond(exchange, 200, bytes)) { (base, requests) =>
                    Using.resource(new HttpDownload(policy)) { downloader =>
                        val store = new ProvenanceStore()
                        val runner = new RetrievalRunner(
                            downloader,
                            new SparkParquetVerifier("local[2]"),
                            store,
                            month => base.resolve(month.value),
                            _ => ()
                        )
                        ...
                        damage match {
                            ...
                            case "invalid-metadata" =>
                                Files.writeString(ProvenanceFiles.metadata(file), "{\"formatVersion\":2}")
                            case "bytes" =>
                                val changed = bytes.clone(); changed(10) = (changed(10) ^ 1).toByte;
        ...
        Fixtures.directory { directory =>
            Fixtures.server { (exchange, number) =>
                ...
                Using.resource(new HttpDownload(policy)) { downloader =>
                    ...
                    assert(runner.run(config(directory)).forall(_.success))
                    val original = Files.readAllBytes(ProvenanceFiles.metadata(file))
                    val results = runner.run(config(directory, refresh = true).copy(months =
    ...
    test("full decoding rejects a corrupt column chunk whose footer is still readable") {
        Fixtures.directory { directory =>
            ...
            val input =
                HadoopInputFile.fromPath(new HadoopPath(file.toUri), spark.sparkContext.hadoopConfiguration)
            val chunk =
                Using.resource(ParquetFileReader.open(input))(_.getFooter.getBlocks.get(0).getColumns.get(1))
            val corrupted = bytes.clone()
            java.util.Arrays.fill(
                corrupted,
                chunk.getStartingPos.toInt,
                (chunk.getStartingPos + chunk.getTotalSize).toInt,
                0.toByte
            )
            Files.write(file, corrupted)
            assert(Using.resource(ParquetFileReader.open(input))(_.getFooter.getBlocks.get(0).getRowCount) ==
                10L)
    ...
    test("HTTP failure during refresh preserves the accepted pair") {
        Fixtures.directory { directory =>
            Fixtures.server((exchange, number) =>
                Fixtures.respond(exchange, if (number == 1) 200 else 503, bytes)
            ) { (base, _) =>
                Using.resource(new HttpDownload(policy)) { downloader =>
                    val store = new ProvenanceStore()
                    val runner = new RetrievalRunner(
            ...
            Fixtures.server((exchange, _) => Fixtures.respond(exchange, 200, bytes)) { (base, requests) =>
                Using.resource(new HttpDownload(policy)) { downloader =>
                    ...
                    val runner = new RetrievalRunner(
                        downloader,
                        interrupted,
                        source = month => base.resolve(month.value),
                        log = _ => ()
                    )
                    intercept[InterruptedException](runner.run(config(directory).copy(months =
                        Month.selected
                    )))
