...
class ReceiptCodecSpec extends AnyFunSuite {
  ...
  test("format version 1 keeps its field names and order") {
    val verification = Verification("message schema {}", """{"type":"struct","fields":[]}""", 10L,
      "2026-09-22T00:00:00Z", "21", "2.13", "4.2.0")
    val entry = ReceiptEntry(StorageKey("nyc_raw/a"), 3L, "a" * 64, "uploaded", "2026-09-22T00:00:01Z",
      mapper.readTree("""{"month":"2026-05"}"""), ProvenanceEntry(StorageKey("nyc_metadata/p"), 2L, "b" * 64, "reused"),
      Some(verification))
    val receipt = Receipt("nyc-taxi", URI.create("http://localhost:9000"), "S", Vector(Month("2026-05")),
      Vector(entry), "R", Instant.parse("2026-09-22T00:00:02Z"), RuntimeVersions("21", "2.13", "4.2.0"))
    val json = mapper.readTree(ReceiptCodec.toJson(receipt))
    assert(json.fieldNames().asScala.toVector == Vector("formatVersion", "bucket", "endpoint", "snapshotId",
      "months", "objects", "publicationId", "completedAt", "versions"))
