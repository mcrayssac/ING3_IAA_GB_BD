...
class RustFsUploadIntegrationSpec extends AnyFunSuite {
    private def fixture(action: (UploadConfig, S3aObjectStore, S3Client) => Unit): Unit =
        Fixtures.directory { root =>
            val bucket = "m23-test-" + UUID.randomUUID().toString
            val config = UploadConfig.parse(
                Array.empty,
                sys.env ++ Map("S3_BUCKET" -> bucket, "SPARK_MASTER" -> "local[2]"),
                root = root
            )
            ...
            try { store.ensureBucket(); action(config, store, client) }
            finally {
                store.close()
                try {
                    // Never run a broad purge against nyc-taxi or a caller-supplied bucket.
                    val uploads = client.listMultipartUploads(
                        ListMultipartUploadsRequest.builder().bucket(bucket).build()
                    )
                    uploads.uploads().asScala.foreach(upload =>
    ...
    test("conditional S3A creation protects existing keys and competing single and multipart writes") {
        fixture { (_, store, _) =>
            ...
            assert(publisher.publish(StorageKey("existing"), payload) == "reused" &&
                store.stat(StorageKey("existing")) == original)
            val changed = Payload.memory(Array[Byte](3, 2, 1))
            intercept[PublicationConflict](publisher.publish(StorageKey("existing"), changed))
            for (blocks <- Vector(1, 70)) {
                ...
                try {
                    for (_ <- 0 until blocks) {
                        first.output.write(block); second.output.write(block.map(b => (b + 1).toByte))
                    }
                    first.commit()
    ...
    test("aborting buffered and multipart streams exposes no partial object or pending upload") {
        fixture { (config, store, client) =>
            for (blocks <- Vector(1, 70)) {
                ...
                try for (_ <- 0 until blocks) pending.output.write(block)
                finally pending.abort()
                assert(store.stat(key).isEmpty)
            }
            val uploads = ListMultipartUploadsRequest.builder().bucket(config.bucket).build()
            assert(client.listMultipartUploads(uploads).uploads().isEmpty)
        }
    }

    test(
        "remote Parquet decoding verifies all columns and rejects corrupt column bytes with a readable footer"
    ) {
        ...
        fixture { (config, store, _) =>
            ...
            try {
                ...
                assert(actual.rowCount == 10 && actual.sparkSchema == expected.sparkSchema &&
                    actual.parquetSchema == expected.parquetSchema)
                Fixtures.directory { root =>
                    ...
                    val chunk = Using.resource(
                        ParquetFileReader.open(input)
                    )(_.getFooter.getBlocks.get(0).getColumns.get(1))
                    ...
                    val remote = HadoopInputFile.fromPath(
                        new HadoopPath(s"s3a://${config.bucket}/corrupt.parquet"),
                        S3aObjectStore.configuration(S3aObjectStore.settings(config))
                    )
                    assert(Using.resource(
                        ParquetFileReader.open(remote)
                    )(_.getFooter.getBlocks.get(0).getRowCount) == 10)
