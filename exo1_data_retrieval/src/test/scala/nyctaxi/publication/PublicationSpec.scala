...
import nyctaxi.shared.{Fixtures, Verification}
...
class PublicationSpec extends AnyFunSuite with BeforeAndAfterAll {
    ...
    private def verifier: RemoteVerifier = new RemoteVerifier {
        override def verify(key: StorageKey): Verification = fixture._2
    }
    private def runner(
        store: ObjectStore,
        inputs: PublicationInputs,
        remote: RemoteVerifier = verifier
    ): PublicationRunner =
    ...
    test("configuration shares strict validation, supports positional precedence, and redacts credentials") {
        Fixtures.directory { root =>
            ...
            assert(defaults.master == "local[2]" && defaults.months == Month.selected)
            val chosen = UploadConfig.parse(
                Array("2026-07", "with spaces"),
                Map("TAXI_MONTHS" -> "invalid", "RAW_DIR" -> ""),
                positional = true,
                root = root
            )
            assert(chosen.months == Vector(Month("2026-07")) && chosen.rawDir == root.resolve("with spaces"))
            for (
                args <- Vector(
                    Array("--refresh"),
                    Array("--help", "2026-05"),
                    Array("2026-05", "x", "y"),
                    Array("2026-05,2026-05")
                )
            )
                intercept[IllegalArgumentException](UploadConfig.parse(
                    args,
                    Map.empty,
                    positional = true,
                    root = root
            ...
            for (
                values <- Vector(
                    Map("SPARK_MASTER" -> "spark://master:7077"),
                    Map("S3_ENDPOINT" -> "http://secret@localhost"),
                    Map("S3_BUCKET" -> "Bad_Bucket"),
                    Map("S3_ACCESS_KEY" -> ""),
                    Map("S3_SECRET_KEY" -> "")
                )
            )
    ...
    test("publication verifies sources and metadata, then reruns write only a new receipt") {
        Fixtures.directory { root =>
            ...
            var reads = 0
            val remote = new RemoteVerifier {
                override def verify(key: StorageKey): Verification = { reads += 1; fixture._2 }
            }
            val first = runner(store, inputs, remote).run(config(root))
            assert(first.success && first.results.size == 3)
            val accepted =
                store.entries.filterNot(_._1.contains("/publications/")).view.mapValues(_.clone()).toMap
            ...
            val receipt = new ObjectMapper().readTree(store.entries(second.receipt.get.value))
            assert(receipt.path("objects").size() == 3 &&
                receipt.path("objects").get(0).path("action").asText() == "reused")
            val receiptText = new String(store.entries(second.receipt.get.value), "UTF-8")
            assert(!receiptText.contains(config(root).secretKey))
        }
    }

    test("invalid local provenance fails its month while later months and references continue") {
        for (damage <- Vector("missing", "sidecar", "checksum", "pending", "schema"))
            Fixtures.directory { root =>
                val inputs = stage(root, fixture._1, fixture._2)
                val file = root.resolve("raw").resolve(StorageLayout.tripFilename(Month("2026-05")))
                damage match {
                    case "missing" => Files.delete(file)
                    case "sidecar" =>
                        Files.delete(file.resolveSibling(file.getFileName.toString + ".metadata.json"))
                    case "checksum" => Files.write(file, Array[Byte](1, 2, 3))
                    case "pending" => Files.writeString(
                            file.resolveSibling(file.getFileName.toString + ".pending"),
                            "interrupted"
                        )
                    case "schema" => ()
                }
                ...
                val remote = if (damage == "schema") new RemoteVerifier {
                    override def verify(key: StorageKey): Verification =
                        if (key.value.contains("2026-05")) fixture._2.copy(rowCount = 99) else fixture._2
                }
                else verifier
                val report = runner(store, inputs, remote).run(config(root).copy(months =
                    Vector(Month("2026-05"), Month("2026-06"))
                ))
                assert(!report.success && report.receipt.isEmpty &&
                    report.results.map(_.success) == Vector(false, true, true, true))
    ...
    test("a reference checksum conflict prevents its upload and the receipt") {
        Fixtures.directory { root =>
            val inputs = stage(root, fixture._1, fixture._2)
            Files.writeString(
                root.resolve("data/reference/tlc/20260919T204934Z/taxi_zone_lookup.csv"),
                "changed"
            )
    ...
    test("remote size and same-size checksum conflicts never replace accepted objects") {
        for (content <- Vector(Array[Byte](0), fixture._1.map(b => (b ^ 1).toByte)))
            Fixtures.directory { root =>
                val inputs = stage(root, fixture._1, fixture._2)
                val store = new MemoryStore()
                val key = "nyc_raw/" + StorageLayout.tripFilename(Month("2026-05"))
                store.entries(key) = content.clone()
                val report = runner(store, inputs).run(config(root))
                assert(!report.success && store.entries(key).sameElements(content) &&
                    !store.writes.contains(key))
            }
    }
        ...
        Fixtures.directory { root =>
            ...
            val store = new MemoryStore() {
                ...
                override def create(key: StorageKey): ObjectWrite =
                    if (fail && key.value.contains("/trips/")) throw new IOException("temporary outage")
            ...
            override def create(key: StorageKey): ObjectWrite = {
                ...
                new ObjectWrite {
                    ...
                    override def commit(): Unit = {
                        underlying.commit(); throw new IOException("response lost")
                    }
                }
            }
        }
        val publisher = new ObjectPublisher(store, new StorageOperations(policy))
        assert(publisher.publish(StorageKey("object"), Payload.memory(Array[Byte](1, 2))) == "reused")
    ...
    test("changing input aborts an incomplete stream without committing or retrying") {
        ...
        val publisher = new ObjectPublisher(store, new StorageOperations(policy))
        intercept[IllegalArgumentException](publisher.publish(StorageKey("object"), payload))
    ...
    test("transient operations have three attempts while access failures stop immediately") {
        ...
        var attempts = 0
        intercept[IOException](operations.run { _ =>
            attempts += 1; throw new IOException("connection lost")
        })
        assert(attempts == 3)
        attempts = 0
        intercept[StorageAccessFailure](operations.run { _ =>
            attempts += 1; throw new AccessDeniedException("denied")
        })
        assert(attempts == 1)
    }
    ...
    test("stalled response bodies hit a deadline and release resources before retry") {
        ...
        val publisher = new ObjectPublisher(store, new StorageOperations(fast))
        intercept[TimeoutException](publisher.publish(StorageKey("object"), Payload.memory(Array[Byte](1))))
    ...
    test("interruption stops later items and raw directory locking excludes another writer") {
        Fixtures.directory { root =>
            ...
            val store = new MemoryStore()
            val remote = new RemoteVerifier {
                override def verify(key: StorageKey): Verification = throw new InterruptedException("stop")
            }
            ...
            val channel = java.nio.channels.FileChannel.open(
                root.resolve("raw/.retrieval.lock"),
                java.nio.file.StandardOpenOption.WRITE
            )
            val lock = channel.lock()
            val blocked = runner(store, inputs)
            try intercept[java.nio.channels.OverlappingFileLockException](blocked.run(config(root)))
