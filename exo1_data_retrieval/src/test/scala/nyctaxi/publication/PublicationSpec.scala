...
class PublicationSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("publication verifies sources and metadata, then reruns write only a new receipt") {
    Fixtures.directory { root =>
      ...
      var reads = 0
      val remote = new RemoteVerifier { override def verify(key: StorageKey): Verification = { reads += 1; fixture._2 } }
      ...
      assert(!new String(store.entries(second.receipt.get.value), "UTF-8").contains(config(root).secretKey))
    }
  }
  ...
  test("invalid local provenance fails its month while later months and references continue") {
    for (damage <- Vector("missing", "sidecar", "checksum", "pending", "schema")) Fixtures.directory { root =>
      ...
      val remote = if (damage == "schema") new RemoteVerifier {
        override def verify(key: StorageKey): Verification = if (key.value.contains("2026-05")) fixture._2.copy(rowCount = 99) else fixture._2
      } else verifier
      val report = runner(store, inputs, remote).run(config(root).copy(months = Vector(Month("2026-05"), Month("2026-06"))))
    ...
    Fixtures.directory { root =>
      ...
      val store = new MemoryStore() {
        ...
        override def create(key: StorageKey): ObjectWrite =
          if (fail && key.value.contains("/trips/")) throw new IOException("temporary outage") else super.create(key)
      }
  ...
  test("lost commit responses are reconciled from remote bytes before any second write") {
    ...
    assert(new ObjectPublisher(store, new StorageOperations(policy)).publish(StorageKey("object"), Payload.memory(Array[Byte](1, 2))) == "reused")
  ...
  test("changing input aborts an incomplete stream without committing or retrying") {
    ...
    intercept[IllegalArgumentException](new ObjectPublisher(store, new StorageOperations(policy)).publish(StorageKey("object"), payload))
  ...
  test("stalled response bodies hit a deadline and release resources before retry") {
    ...
    intercept[TimeoutException](new ObjectPublisher(store, new StorageOperations(fast)).publish(StorageKey("object"), Payload.memory(Array[Byte](1))))
  ...
  test("interruption stops later items and raw directory locking excludes another writer") {
    Fixtures.directory { root =>
      val inputs = stage(root, fixture._1, fixture._2)
      val store = new MemoryStore()
      val remote = new RemoteVerifier { override def verify(key: StorageKey): Verification = throw new InterruptedException("stop") }
