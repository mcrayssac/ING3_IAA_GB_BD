...
class MainSpec extends AnyFunSuite {
    ...
    test("positional values override only supplied environment settings") {
        ...
        assert(overrideBoth.rawDir == root.resolve("data/path with spaces") &&
            overrideBoth.master == "local[2]")
    ...
    test("invalid arguments and configuration fail before execution") {
        for (
            args <- Vector(
                Array("--unknown"),
                Array("--refresh", "2026-05"),
                Array("--refresh", "--refresh"),
                Array("--help", "2026-05"),
                Array("2026-05", "data/raw", "extra"),
                Array("2026-05", ""),
        ...
        intercept[IllegalArgumentException](Main.parse(
            Array.empty,
