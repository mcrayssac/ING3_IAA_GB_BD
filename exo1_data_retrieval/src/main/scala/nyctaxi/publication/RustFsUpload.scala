...
object RustFsUpload {
    ...
    def execute(configuration: () => UploadConfig): Int = {
        val config = try configuration()
            ...
            case NonFatal(_) =>
                System.err.println("Invalid upload configuration. Run upload --help."); return 2
        }
        ...
        try {
            ...
            println(
                s"Summary: ${report.results.count(_.success)}/${report.results.size} source files verified"
            )
            ...
            try if (verifier != null) verifier.close()
