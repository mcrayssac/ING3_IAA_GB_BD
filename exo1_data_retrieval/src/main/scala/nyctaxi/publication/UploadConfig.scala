...
final case class UploadConfig(
  months: Vector[Month], rawDir: Path, root: Path, master: String,
...
object UploadConfig {
  ...
  def parse(args: Array[String], env: Map[String, String], positional: Boolean = false,
    ...
    UploadConfig(local.months, local.rawDir, root.toAbsolutePath.normalize(), local.master,
      endpoint(values), access, secret, bucket(values))
  }
  ...
  private def endpoint(values: Map[String, String]): URI = {
    ...
    require(Set("http", "https").contains(uri.getScheme) && uri.getHost != null &&
  ...
  private def bucket(values: Map[String, String]): String = {
    ...
    require(name.matches("[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]") && !name.contains("..") &&
