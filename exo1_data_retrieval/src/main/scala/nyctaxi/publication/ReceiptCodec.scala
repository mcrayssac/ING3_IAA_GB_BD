...
object ReceiptCodec {
  ...
  private def writeEntry(node: ObjectNode, entry: ReceiptEntry): Unit = {
    ...
    node.putObject("provenance").put("key", entry.provenance.key.value).put("bytes", entry.provenance.bytes)
      .put("sha256", entry.provenance.sha256).put("action", entry.provenance.action)
    entry.verification.foreach { checked =>
      ...
      verification.putObject("versions").put("java", checked.javaVersion).put("scala", checked.scalaVersion)
