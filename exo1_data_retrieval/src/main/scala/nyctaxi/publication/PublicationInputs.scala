...
object Payload {
    ...
    def file(path: Path, size: Long, sha256: String): Payload = {
        require(
            Files.isRegularFile(path) && Files.size(path) == size && Digest.sha256(path) == sha256,
...
final class PublicationInputs(descriptor: Array[Byte] = PublicationInputs.descriptor) {
    ...
    // LIMIT: snapshot pinned to 20260919T204934Z. A new snapshot needs a new descriptor and release.
    require(
        snapshot.path("formatVersion").asInt() == 1 &&
            snapshot.path("snapshotId").asText() == "20260919T204934Z",
    ...
    require(
        referenceFiles.map(_.path("filename").asText()) == expectedReferences,
    ...
    private def tripInput(config: UploadConfig, month: Month): (String, () => PublicationInput) = {
        ...
        filename ->
            (() => {
                ...
                PublicationInput(
                    StorageLayout.rawTrip(month),
                    Payload.file(file, saved.bytes, saved.sha256),
                    mapper.readTree(sidecar),
                    StorageLayout.tripProvenance(month, provenance.sha256),
                    provenance,
    ...
    private def referenceInput(
        ...
        filename ->
            (() =>
                PublicationInput(
                    StorageLayout.reference(snapshotId, filename),
                    Payload.file(
                        StorageLayout.localReference(config.root, snapshotId, filename),
                        reference.path("bytes").asLong(),
                        reference.path("sha256").asText()
                    ),
                    reference,
                    StorageLayout.referenceDescriptor(snapshotId),
                    Payload.memory(descriptor),
                    None
                )
            )
    }
