...
import java.util.concurrent.{
    Callable, CountDownLatch, ExecutionException, Executors, TimeUnit,
    TimeoutException
}
...
object StorageFailures {
    private def causes(error: Throwable): Vector[Throwable] = {
        ...
        var current = error
        while (current != null && !result.exists(_ eq current)) {
            result += current; current = current.getCause
        }
        result.toVector
    }
...
final class StorageOperations(policy: StoragePolicy = StoragePolicy()) {
    ...
    private def once[A](operation: OperationScope => A): A = {
        ...
        val future = executor.submit(new Callable[A] {
            override def call(): A = try operation(scope)
            finally finished.countDown()
        })
        try future.get(policy.operationTimeout.toMillis, TimeUnit.MILLISECONDS)
        catch {
