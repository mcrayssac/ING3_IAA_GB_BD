...
final class PublicationRunner(
  ...
  /** Publishes selected sources, then writes a receipt only if every item verified.
    * Steps: lock RAW_DIR -> ensure bucket -> per item (upload, verify remote, upload provenance) -> receipt.
    */
  ...
  /** Publishes one source. This is the single catch point turning an item failure into a redacted result. */
  private def publishItem(config: UploadConfig, name: String, load: () => PublicationInput): PublishedItem = {
  ...
  private def publishVerified(name: String, input: PublicationInput): ReceiptEntry = {
    ...
    val provenance = ProvenanceEntry(input.provenanceKey, input.provenance.bytes, input.provenance.sha256,
      provenanceAction)
    ReceiptEntry(input.key, input.payload.bytes, input.payload.sha256, action, Instant.now().toString,
      input.source, provenance, verification)
  }
  ...
  private def writeReceipt(config: UploadConfig, entries: Vector[ReceiptEntry]): StorageKey = {
    ...
    val receipt = Receipt(config.bucket, config.endpoint, inputs.snapshotId, config.months, entries, id,
      completedAt, RuntimeVersions.current)
