...
object ProvenanceCodec {
  ...
  def read(path: Path): Provenance = {
    ...
    require(node != null && node.isObject && number(node, "formatVersion") == 1L,
...
object ProvenanceFiles {
  ...
  def existing(file: Path, month: Month, sourceUrl: String): Option[Provenance] = {
    ...
    require(saved.month == month.value && saved.filename == file.getFileName.toString &&
      saved.sourceUrl == sourceUrl, "Metadata does not describe the requested source. Use --refresh")
    require(saved.bytes == Files.size(file) && saved.sha256 == Digest.sha256(file),
