...
import nyctaxi.shared.{
    Interrupts, ItemResult, ParquetVerifier, Provenance, ProvenanceFiles, RawDirLock,
...
final class RetrievalRunner(
    ...
    def run(config: RetrievalConfig): Vector[ItemResult] = {
        ...
        RawDirLock.hold(config.rawDir, "Another retrieval process is using RAW_DIR") {
            config.months.map { month =>
                ...
                val result = try retrieve(config, month)
                catch {
                    case NonFatal(error) =>
                        ItemResult(
                            month.value,
    ...
    private def retrieve(config: RetrievalConfig, month: Month): ItemResult = {
        ...
        ItemResult(
            month.value,
            true,
            s"$action and fully verified $name, ${Files.size(file)} bytes, ${report.rowCount} rows"
        )
    }
}
    ...
    def execute(configuration: => RetrievalConfig): Int = {
        ...
        val status = try {
            ...
            case _: InterruptedException =>
                Thread.currentThread().interrupt(); System.err.println("Retrieval interrupted"); 130
