...
lazy val root = (project in file("."))
    .settings(
        ...
        retrieve := {
            val requested =
                sbt.complete.DefaultParsers.spaceDelimited("[months] [rawDir] [--refresh] | --help").parsed
        ...
        upload := {
            ...
            val options = ForkOptions()
                ...
                .withOutputStrategy(Some(LoggedOutput(streams.value.log)))
            val code =
                Fork.java(options, Seq("-cp", classpath, "nyctaxi.publication.UploadMain") ++ requested)
